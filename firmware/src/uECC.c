 ...
 * @brief Implementation of the Micro-ECC library.
 *
 * This file provides the core mathematical and cryptographic routines for
 * ECDH and ECDSA, specifically optimized for the MSPM0 platform.
 */
...
#if (uECC_CURVE == uECC_secp160r1 || uECC_CURVE == uECC_secp224r1) &&          \
    (uECC_WORD_SIZE == 8)
...
#define Curve_B_1                                                              \
    {0x45, 0xFA, 0x65, 0xC5, 0xAD, 0xD4, 0xD4, 0x81, 0x9F, 0xF8,               \
     0xAC, 0x65, 0x8B, 0x7A, 0xBD, 0x54, 0xFC, 0xBE, 0x97, 0x1C}
#define Curve_B_2                                                              \
...
#define Curve_G_1                                                              \
    {{0x82, 0xFC, 0xCB, 0x13, 0xB9, 0x8B, 0xC3, 0x68, 0x89, 0x69,              \
      0x64, 0x46, 0x28, 0x73, 0xF5, 0x8E, 0x68, 0xB5, 0x96, 0x4A},             \
     {0x32, 0xFB, 0xC5, 0x7A, 0x37, 0x51, 0x23, 0x04, 0x12, 0xC9,              \
      0xDC, 0x59, 0x7D, 0x94, 0x68, 0x31, 0x55, 0x28, 0xA6, 0x23}}
...
#define Curve_N_1                                                              \
    {0x57, 0x22, 0x75, 0xCA, 0xD3, 0xAE, 0x27, 0xF9, 0xC8, 0xF4, 0x01,         \
...
#define Curve_P_1                                                              \
    {0xFFFFFFFF7FFFFFFFull, 0xFFFFFFFFFFFFFFFFull, 0x00000000FFFFFFFFull}
#define Curve_P_2                                                              \
...
#define Curve_B_1                                                              \
    {0x81D4D4ADC565FA45ull, 0x54BD7A8B65ACF89Full, 0x000000001C97BEFCull}
#define Curve_B_2                                                              \
    {0xFEB8DEECC146B9B1ull, 0x0FA7E9AB72243049ull, 0x64210519E59C80E7ull}
#define Curve_B_3                                                              \
    {0x3BCE3C3E27D2604Bull, 0x651D06B0CC53B0F6ull, 0xB3EBBD55769886BCull,      \
     0x5AC635D8AA3A93E7ull}
#define Curve_B_4                                                              \
    {0x0000000000000007ull, 0x0000000000000000ull, 0x0000000000000000ull,      \
     0x0000000000000000ull}
#define Curve_B_5                                                              \
    {0x270B39432355FFB4ull, 0x5044B0B7D7BFD8BAull, 0x0C04B3ABF5413256ull,      \
...
#define Curve_G_5                                                              \
    {{0x343280D6115C1D21ull, 0x4A03C1D356C21122ull, 0x6BB4BF7F321390B9ull,     \
      0x00000000B70E0CBDull},                                                  \
     {0x44D5819985007E34ull, 0xCD4375A05A074764ull, 0xB5F723FB4C22DFE6ull,     \
      0x00000000BD376388ull}}

#define Curve_N_1                                                              \
    {0xF927AED3CA752257ull, 0x000000000001F4C8ull, 0x0000000100000000ull}
#define Curve_N_2                                                              \
...
#define Curve_N_4                                                              \
    {0xBFD25E8CD0364141ull, 0xBAAEDCE6AF48A03Bull, 0xFFFFFFFFFFFFFFFEull,      \
     0xFFFFFFFFFFFFFFFFull}
#define Curve_N_5                                                              \
    {0x13DD29455C5C2A3Dull, 0xFFFF16A2E0B8F03Eull, 0xFFFFFFFFFFFFFFFFull,      \
     0x00000000FFFFFFFFull}
...
static uECC_word_t vli_add(uECC_word_t *result, const uECC_word_t *left,
                           const uECC_word_t *right);
static uECC_word_t vli_sub(uECC_word_t *result, const uECC_word_t *left,
                           const uECC_word_t *right);
static void vli_mult(uECC_word_t *result, const uECC_word_t *left,
                     const uECC_word_t *right);
static void vli_modAdd(uECC_word_t *result, const uECC_word_t *left,
                       const uECC_word_t *right, const uECC_word_t *mod);
static void vli_modInv(uECC_word_t *result, const uECC_word_t *input,
...
static void vli_modMult_n(uECC_word_t *result, const uECC_word_t *left,
                          const uECC_word_t *right);
static void vli_modAdd_n(uECC_word_t *result, const uECC_word_t *left,
                         const uECC_word_t *right, const uECC_word_t *mod);
static void vli_modInv_n(uECC_word_t *result, const uECC_word_t *input,
                         const uECC_word_t *mod);
static void vli_rshift1_n(uECC_word_t *vli);
static uECC_word_t vli_add_n(uECC_word_t *result, const uECC_word_t *left,
                             const uECC_word_t *right);
static uECC_word_t vli_sub_n(uECC_word_t *result, const uECC_word_t *left,
                             const uECC_word_t *right);
#else
#define vli_clear_n vli_clear
#define vli_set_n vli_set
#define vli_cmp_n vli_cmp
#define vli_modMult_n vli_modMult_generic
#define vli_modAdd_n vli_modAdd
#define vli_modInv_n vli_modInv
#define vli_rshift1_n vli_rshift1
#define vli_add_n vli_add
#define vli_sub_n vli_sub
#endif
...
#elif defined(unix) || defined(__linux__) || defined(__unix__) ||              \
...
/* Computes result = (left - right) % mod.
   ...
   mod. */
#if !asm_modSub
static void vli_modSub(uECC_word_t *result, const uECC_word_t *left,
                       const uECC_word_t *right, const uECC_word_t *mod)
{
 ...
 */
static void omega_mult(uECC_word_t *RESTRICT result,
                       const uECC_word_t *RESTRICT right);

/* Computes result = product % curve_p
    see http://www.isys.uni-klu.ac.at/PDF/2001-0126-MT.pdf page 354

    Note that this only works if log2(omega) < log2(p) / 2 */
static void vli_mmod_fast(uECC_word_t *RESTRICT result,
                          uECC_word_t *RESTRICT product)
{
    uECC_word_t tmp[2 * uECC_WORDS];
    uECC_word_t carry;
    uECC_word_t dummy[uECC_WORDS];
    uECC_word_t borrow_out;
    uECC_word_t must_sub;
    uECC_word_t mask;
    int i, j;
    ...
    for (i = 0; i < 4; ++i)
    {
        borrow_out = vli_sub(dummy, result, curve_p);

        must_sub = (carry > 0) | (borrow_out == 0);
        mask = 0 - (uECC_word_t) (must_sub & 1); // 0 or 0xFFFFFFFF

        for (j = 0; j < uECC_WORDS; ++j)
        {
            result[j] = (result[j] & ~mask) | (dummy[j] & mask);
        }
...
#elif uECC_WORD_SIZE == 4
static void omega_mult(uint32_t *RESTRICT result,
                       const uint32_t *RESTRICT right)
{
    uint32_t carry;
    unsigned i;
...
static void omega_mult(uint8_t *RESTRICT result, const uint8_t *RESTRICT right)
{
    ...
    result[4 + uECC_WORDS] =
        vli_add(result + 4, result + 4, right); /* add the 2^32 multiple */
}
#elif uECC_WORD_SIZE == 4
static void omega_mult(uint32_t *RESTRICT result,
                       const uint32_t *RESTRICT right)
{
    ...
    result[1 + uECC_WORDS] =
        vli_add(result + 1, result + 1, right); /* add the 2^32 multiple */
}
#else
static void omega_mult(uint64_t *RESTRICT result,
                       const uint64_t *RESTRICT right)
{
    uECC_word_t r0 = 0;
    uECC_word_t r1 = 0;
    uECC_word_t r2 = 0;
...
static void vli_modMult_fast(uECC_word_t *result, const uECC_word_t *left,
                             const uECC_word_t *right)
{
...
static void mod_sqrt_secp224r1_rm(uECC_word_t *d2, uECC_word_t *e2,
                                  ...
                                  const uECC_word_t *d1, const uECC_word_t *e1)
{
    uECC_word_t t1[uECC_WORDS];
...
static void vli_nativeToBytes(uint8_t *RESTRICT dest,
...
static void curve_x_side(uECC_word_t *RESTRICT result,
                         const uECC_word_t *RESTRICT x)
{
#if (uECC_CURVE == uECC_secp256k1)
...
int uECC_compute_public_key(const uint8_t private_key[uECC_BYTES],
                            uint8_t public_key[uECC_BYTES * 2])
{
    uECC_word_t private[uECC_N_WORDS];
    EccPoint public;

    vli_clear(private + uECC_WORDS);
                             ...
                             const uECC_word_t *right)
{
    ...
    uECC_word_t diff =
        left[uECC_N_WORDS - 1] - right[uECC_N_WORDS - 1] - borrow;
...
static void vli_mult_n(uECC_word_t *result, const uECC_word_t *left,
                       const uECC_word_t *right)
{
    uECC_word_t r0 = 0;
    uECC_word_t r1 = 0;
...
static void vli_modInv_n(uECC_word_t *result, const uECC_word_t *input,
                         const uECC_word_t *mod)
{
    uECC_word_t a[uECC_N_WORDS], b[uECC_N_WORDS], u[uECC_N_WORDS],
        v[uECC_N_WORDS];
    uECC_word_t carry;
    cmpresult_t cmpResult;
...
static uECC_word_t vli2_sub_n(uECC_word_t *result, const uECC_word_t *left,
                              const uECC_word_t *right)
{
    uECC_word_t borrow = 0;
    wordcount_t i;
                          ...
                          const uECC_word_t *right)
{
    ...
    for (i = 0; i <= ((((bitcount_t) uECC_N_WORDS) << uECC_WORD_BITS_SHIFT) +
                      (uECC_WORD_BITS - 1));
         ++i)
    {
...
static void vli_modMult_generic(uECC_word_t *result, const uECC_word_t *left,
                                const uECC_word_t *right)
{
    uECC_word_t product[2 * uECC_WORDS];
    ...
    vli_mult(product, left, right);
    vli_set(modMultiple + uECC_WORDS, curve_n);
...
static void vli_blindScalar(uECC_word_t *result, const uECC_word_t *scalar)
{
    ...
    for (tries = 0; tries < MAX_TRIES; ++tries)
    {
        if (g_rng_function((uint8_t *) &r, sizeof(r)) && r != 0)
        {
            break;
        }
    }
    ...
    uECC_dword_t carry = 0;
    wordcount_t i;
    for (i = 0; i < uECC_N_WORDS; ++i)
    {
        uECC_dword_t p = (uECC_dword_t) r * curve_n[i] + scalar[i] + carry;
        result[i] = (uECC_word_t) p;
        carry = p >> uECC_WORD_BITS;
    }
    result[uECC_N_WORDS] = (uECC_word_t) carry;
}

static int EccPoint_compute_public_key(EccPoint *result, uECC_word_t *private)
{
    uECC_word_t blinded[uECC_N_WORDS + 1];
    uECC_word_t random_Z[uECC_WORDS];
    uECC_word_t *initial_Z = 0;
    uECC_word_t scalar_n[uECC_N_WORDS];

    vli_clear_n(scalar_n);
    vli_set(scalar_n, private);

    if (vli_isZero(scalar_n) || vli_cmp_n(curve_n, scalar_n) != 1)
    {
        return 0;
    }

    if (g_rng_function((uint8_t *) random_Z, sizeof(random_Z)) &&
        !vli_isZero(random_Z))
    {
        initial_Z = random_Z;
    }

    vli_blindScalar(blinded, scalar_n);
    EccPoint_mult(result, &curve_G, blinded, initial_Z,
                  vli_numBits(blinded, uECC_N_WORDS + 1));
                       ...
                       uint8_t secret[uECC_BYTES])
{
    ...
    for (tries = 0; tries < MAX_TRIES; ++tries)
    {
        if (g_rng_function((uint8_t *) random, sizeof(random)) &&
            !vli_isZero(random))
        {
            initial_Z = random;
            break;
    ...
    vli_blindScalar(blinded, private);
    EccPoint_mult(&product, &public, blinded, initial_Z,
                  vli_numBits(blinded, uECC_N_WORDS + 1));

    vli_nativeToBytes(secret, product.x);
    if (EccPoint_isZero(&product))
    {
        return 0;
    }

    uint8_t check_point[uECC_BYTES * 2];
    vli_nativeToBytes(check_point, product.x);
    vli_nativeToBytes(check_point + uECC_BYTES, product.y);
    if (!uECC_valid_public_key(check_point))
    {
        return 0;
    }
                            ...
                            uint8_t signature[uECC_BYTES * 2])
{
    ...
    /* p = k * G */
    vli_blindScalar(blinded, k);
    EccPoint_mult(&p, &curve_G, blinded, 0,
                  vli_numBits(blinded, uECC_N_WORDS + 1));
    ...
    // If the RNG fails every time (eg it was not defined), we continue so
    // that deterministic signing can still work (with reduced security)
    // without an RNG defined.
              ...
              uint8_t signature[uECC_BYTES * 2])
{
    ...
    for (tries = 0; tries < MAX_TRIES; ++tries)
    {
        if (g_rng_function((uint8_t *) k, sizeof(k)))
        {
            ...
            {
                uint8_t public_key[uECC_BYTES * 2];
                if (!uECC_compute_public_key(private_key, public_key))
                {
                    return 0;
                }
                if (!uECC_verify(public_key, message_hash, signature))
                {
                    return 0;
                }
                return 1;
...
static void HMAC_finish(uECC_HashContext *hash_context, const uint8_t *K,
                        uint8_t *result)
{
    uint8_t *pad = hash_context->tmp + 2 * hash_context->result_size;
    unsigned i;
                            ...
                            uint8_t signature[uECC_BYTES * 2])
{
    ...
    for (tries = 0; tries < MAX_TRIES; ++tries)
    {
        ...
        {
            update_V(hash_context, K, V);
            for (i = 0; i < hash_context->result_size && T_bytes < sizeof(T);
                 ++i, ++T_bytes)
            {
                T_ptr[T_bytes] = V[i];
            }
                ...
                const uint8_t signature[uECC_BYTES * 2])
{
...
#if (uECC_CURVE != uECC_secp160r1)
    if (vli_cmp_n(curve_n, r) != 1 || vli_cmp_n(curve_n, s) != 1)
    { /* r, s must be < n. */
        return 0;
    }
#endif
    ...
    points[3] = &sum;
    numBits =
        smax(vli_numBits(u1, uECC_N_WORDS), vli_numBits(u2, uECC_N_WORDS));

    point = points[(!!vli_testBit(u1, numBits - 1)) |
                   ((!!vli_testBit(u2, numBits - 1)) << 1)];
    vli_set(rx, point->x);
    vli_set(ry, point->y);
    vli_clear(z);
    z[0] = 1;
...
void uECC_mod_mult(uint8_t result[32], const uint8_t left[32],
                   const uint8_t right[32])
{
    uECC_word_t l[uECC_WORDS];
    uECC_word_t r[uECC_WORDS];
    uECC_word_t res[uECC_WORDS];

    vli_bytesToNative(l, left);
    vli_bytesToNative(r, right);

    vli_modMult_n(res, l, r);
